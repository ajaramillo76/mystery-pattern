...
public class PatternView extends View {
  ...
  private Paint paint;
  private RectF dest; //for graphics, you typically work in floats.

  {
    setWillNotDraw(false); //statement only needed in android.
    paint = new Paint();
    dest = new RectF();
  }
  ...
  protected void onDraw(Canvas canvas) {
    if (vertices != null) {
      ...
      for (Position position : vertices) {
        canvas.drawCircle((float) position.getX(), (float) position.getY(), 25, paint);  //if you want to go back to a previous bucket, you must cast it (i.e., (float)).
      }
    }
  }
